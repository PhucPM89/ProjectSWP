 ...
 * @author Admin
 */
public class ShowTableControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ShowTableControl</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ShowTableControl at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (tableIdString == null || tableIdString.isEmpty()) {
            response.sendRedirect("managertable");
            return;
        }
        int tableId = Integer.parseInt(tableIdString);
        dao.showTable(tableId);

        List<Table> listT = dao.getHideTable();
        ms = "Đã hiển thị sản phẩm !";
